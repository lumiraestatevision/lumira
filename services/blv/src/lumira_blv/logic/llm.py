...
async def extract(pdf: bytes, settings: BLVSettings) -> BLVExtraction:
    api_key = settings.api_key
    if api_key is None:
        raise RuntimeError(f"Kein API-Key für Anbieter '{settings.blv_provider}'")
    match settings.blv_provider:
        case "anthropic":
            return await claude.extract_with_claude(
                pdf, api_key=api_key, model=settings.model, use_fallbacks=settings.blv_llm_fallbacks
            )
        case "gemini":
            return await gemini.extract_with_gemini(pdf, api_key=api_key, model=settings.model)
